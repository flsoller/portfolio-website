...
Due to human error, all RDS database endpoint variables for our Kinesis stream processor were misconfigured for the production environment. The staging end-to-end tests had successfully passed and the subsequent deployment to the demo environment was normal. However, during production deployments our error monitors suddenly alerted us to an extreme rate of function errors. Since the database endpoints were not correctly set, all processing attempts would fail due to a DB connection issue. Subsequently, each and every event was routed to our OnFailure S3 bucket.
